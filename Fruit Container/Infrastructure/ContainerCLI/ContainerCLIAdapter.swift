...
enum ContainerCompatibilityState: Sendable {
    case supported
    case untestedNewerMajor(reason: String)
...
struct ImageListItem: Identifiable, Hashable, Sendable {
    ...
    let totalVariantSizeBytes: Int64?
    ...
    init(
        ...
        variantCount: Int = 0,
        platforms: [String] = [],
        totalVariantSizeBytes: Int64? = nil
    ) {
        self.id = id
        ...
        self.totalVariantSizeBytes = totalVariantSizeBytes
    }
}

private struct ImageListRow: Decodable {
    let configuration: ImageListConfiguration?
    let id: String?
    ...
    init(from decoder: Decoder) throws {
        ...
        configuration = try container.decodeIfPresent(ImageListConfiguration.self, forKey: .configuration)
        id = try container.decodeIfPresent(String.self, forKey: .id)
        variants = try container.decodeIfPresent([ImageListVariant].self, forKey: .variants) ?? []
    }
}

private struct ImageListConfiguration: Decodable {
    let creationDate: String?
    let descriptor: ImageListDescriptor
    let name: String?
}

private struct ImageListDescriptor: Decodable {
    let digest: String?
    let mediaType: String?
    let size: Int64?
}

private struct ImageListVariant: Decodable {
    let config: ImageListConfig?
    let digest: String?
    let platform: ImageListPlatform?
    let size: Int64?
}

private struct ImageListConfig: Decodable {
    let architecture: String?
    let config: ImageListRuntimeConfig?
    let created: String?
    let history: [ImageListHistory]
    let os: String?
    let rootfs: ImageListRootfs?
    let variant: String?
    ...
    init(from decoder: Decoder) throws {
        ...
        config = try container.decodeIfPresent(ImageListRuntimeConfig.self, forKey: .config)
        created = try container.decodeIfPresent(String.self, forKey: .created)
        history = try container.decodeIfPresent([ImageListHistory].self, forKey: .history) ?? []
        os = try container.decodeIfPresent(String.self, forKey: .os)
        rootfs = try container.decodeIfPresent(ImageListRootfs.self, forKey: .rootfs)
        variant = try container.decodeIfPresent(String.self, forKey: .variant)
    }
}

private struct ImageListRuntimeConfig: Decodable {
    let cmd: [String]
...
private struct ImageListHistory: Decodable {
    let comment: String?
    let created: String?
...
private struct ImageListPlatform: Decodable {
    let architecture: String?
    let os: String?
    let variant: String?
}

private struct ImageListRootfs: Decodable {
    let diffIDs: [String]
    let type: String?
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        diffIDs = try container.decodeIfPresent([String].self, forKey: .diffIDs) ?? []
        type = try container.decodeIfPresent(String.self, forKey: .type)
    }
}

private extension ImageListRow {
    var platformDisplays: [String] {
        variants.compactMap(\.platformDisplay).uniquedPreservingOrder()
    }

    var totalVariantSizeBytes: Int64? {
        let total = variants.compactMap(\.size).reduce(Int64(0), +)
        return total > 0 ? total : nil
    }
}

private extension ImageListVariant {
    var platformDisplay: String? {
        let os = platform?.os ?? config?.os
        let architecture = platform?.architecture ?? config?.architecture
        guard let os, let architecture else { return nil }

        var parts = [os, architecture]
        if let variant = platform?.variant ?? config?.variant {
...
struct ContainerCLIAdapter: Sendable {
    static let containerListArguments = ["list", "--all", "--format", "json"]
    static let latestSignedInstallerVersion = "1.0.0"
    static let latestSignedInstallerURL = URL(
    ...
    func fetchCommandHelp(arguments: [String]) async throws -> String {
        let executablePath = await resolveContainerExecutable()
        let command = makeContainerCommand(
            executablePath: executablePath,
    ...
    func supportsCommand(_ path: [String]) async -> Bool {
        do {
            let output = try await fetchCommandHelp(arguments: path)
            return Self.commandHelpIndicatesSupported(output, path: path)
        } catch {
            return false
        }
    }

    static func commandHelpIndicatesSupported(_ output: String) -> Bool {
        commandHelpIndicatesSupported(output, path: [])
    }

    static func commandHelpIndicatesSupported(_ output: String, path: [String]) -> Bool {
        ...
        guard !unsupportedFragments.contains(where: { normalized.contains($0) }) else {
            return false
        }
        guard let commandName = path.last?.lowercased(), !commandName.isEmpty else {
            return true
        }

        let escapedCommand = NSRegularExpression.escapedPattern(for: commandName)
        let patterns = [
            #"(?m)^\s*\#(escapedCommand)(?:,|\s)"#,
            #"\bcontainer\s+\#(escapedCommand)\b"#,
        ]
    ...
    func listContainers() async throws -> NonCriticalDecodeResult<[ContainerListItem]> {
        ...
        let command = makeContainerCommand(
            executablePath: executablePath,
            arguments: Self.containerListArguments,
            timeout: 15
        )
        let result = try await commandRunner.run(command)
    ...
    func pullImage(
        ...
        let command = makeContainerCommand(
            executablePath: executablePath,
            arguments: ["image", "pull", "--progress", "plain", "--", trimmed],
            timeout: 180
        )
        if let onOutput {
            _ = try await commandRunner.runStreaming(command, onOutput: onOutput)
        } else {
    ...
    func inspectContainer(id: String) async throws -> ContainerInspectSnapshot {
        ...
        let result = try await commandRunner.run(command)
        return decodeContainerInspect(id: id, output: result.stdout)
    }

    private func decodeContainerInspect(id: String, output: String) -> ContainerInspectSnapshot {
        let prettyJSON = prettyPrintedJSON(from: output) ?? output

        guard
            let rows = parseJSONArrayRows(from: output),
            let first = rows.first
        else {
        ...
        let platform = nestedDictionary(configuration, key: "platform")
        let initProcess = nestedDictionary(configuration, key: "initprocess")
        let configuredNetworkRows = valueDictionaryArray(configuration, key: "networks")
        let firstConfiguredNetwork = configuredNetworkRows.first.map(normalizeKeys) ?? [:]
        let firstConfiguredNetworkOptions = nestedDictionary(firstConfiguredNetwork, key: "options")
        let statusNetworks = valueDictionaryArray(status, key: "networks")
        let firstStatusNetwork = statusNetworks.first.map(normalizeKeys) ?? [:]
        let processCommand = commandText(from: initProcess)
        let configuredNetworks = configuredNetworkRows.compactMap { valueString(normalizeKeys($0), keys: ["network", "name", "id"]) }
        ...
        return ContainerInspectSnapshot(
            containerID: id,
            rawJSON: prettyJSON,
            state: valueString(status, keys: ["state"]),
            imageReference: valueString(image, keys: ["reference"]),
            hostname: valueString(firstConfiguredNetworkOptions, keys: ["hostname"])
                ?? valueString(firstConfiguredNetwork, keys: ["hostname"]),
            networkAddress: valueString(firstStatusNetwork, keys: ["ipv4address", "ipv6address", "address", "ipaddress"]),
            platform: containerPlatformDisplay(from: platform),
            command: processCommand,
            environment: valueStringArray(initProcess, key: "environment"),
            cpuCount: valueInt(resources, keys: ["cpus"]),
            memoryBytes: valueInt64(resources, keys: ["memoryinbytes"]),
            publishedPorts: publishedPortDisplays(from: valueDictionaryArray(configuration, key: "publishedports")),
            rosetta: valueBool(configuration, keys: ["rosetta"]),
            readOnly: valueBool(configuration, keys: ["readonly"]),
            runtimeHandler: valueString(configuration, keys: ["runtimehandler"]),
            configuredNetworkNames: configuredNetworks,
            attachedVolumes: attachedVolumes,
            labels: valueStringDictionary(configuration, key: "labels")
        )
    }
    ...
    func fetchContainerStats() async throws -> [ContainerStatsSample] {
        let executablePath = await resolveContainerExecutable()
        let command = makeContainerCommand(
            executablePath: executablePath,
            arguments: ["stats", "--format", "json", "--no-stream"],
            timeout: 15
        )
        let result = try await commandRunner.run(command)
        return decodeContainerStats(from: result.stdout)
    }
    ...
    func fetchContainerStats(id: String) async throws -> ContainerStatsSample? {
        ...
        let result = try await commandRunner.run(command)
        return decodeContainerStats(from: result.stdout, fallbackID: id).first
    }
    ...
    func updateContainerCommand() async throws {
        let scriptPath = "/usr/local/bin/update-container.sh"
        guard FileManager.default.isExecutableFile(atPath: scriptPath) else {
            throw AppError.commandLaunchFailed(
                command: scriptPath,
                reason: "Update script is missing or is not executable."
            )
        }
    ...
    func downloadLatestSignedInstaller() async throws -> URL {
        let downloadDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("FruitContainer", isDirectory: true)
        let packageURL = downloadDirectory
            .appendingPathComponent("container-\(Self.latestSignedInstallerVersion)-installer-signed.pkg")
        ...
        let command = CLICommand(
            ...
            arguments: [
                ...
                "--output",
                packageURL.path,
                Self.latestSignedInstallerURL.absoluteString,
            ],
            timeout: 300
        )
        _ = try await commandRunner.run(command)
    ...
    private func evaluateCompatibility(for version: ContainerSystemVersion) -> ContainerCompatibilityState {
        ...
        if semanticVersion.major > policy.maximumSupportedMajor {
            return .untestedNewerMajor(
                reason:
                    "Installed major version \(semanticVersion.major) is newer than the tested policy range \(policy.supportedRangeDescription). Core commands may still work, but this version needs smoke testing."
            )
        }
    ...
    func evaluateCompatibilityForTesting(_ rawVersion: String) -> ContainerCompatibilityState {
        evaluateCompatibility(
            for: ContainerSystemVersion(
                version: rawVersion,
                buildType: "test",
                commit: "test",
                appName: "container",
    ...
    private func runRegistryLoginWithPasswordStdin(
        ...
        let command = CLICommand(
            executable: executablePath ?? "/usr/bin/container",
            arguments: arguments,
    ...
    private func decodeContainerList(from output: String) -> NonCriticalDecodeResult<[ContainerListItem]> {
        ...
        for row in rows {
            ...
            let platform = nestedDictionary(configuration, key: "platform")
            let initProcess = nestedDictionary(configuration, key: "initprocess")
            let labels = valueStringDictionary(configuration, key: "labels")
            let networks = (configuration["networks"] as? [[String: Any]]) ?? (lookup["networks"] as? [[String: Any]])
            let firstNetwork = networks?.first.map(normalizeKeys)
            let networkOptions = nestedDictionary(firstNetwork, key: "options")

            guard
                let id = valueString(lookup, keys: ["id", "containerid"])
                    ?? valueString(configuration, keys: ["id", "containerid"]),
                let state = valueString(status, keys: ["state"])
                    ?? valueString(lookup, keys: ["state", "status"])
            else {
            ...
            let name =
                ...
                ?? valueString(networkOptions, keys: ["hostname"])
                ?? valueString(firstNetwork ?? [:], keys: ["hostname"])
                ?? valueString(configuration, keys: ["id"])
                ?? id
            let platformDisplay = containerPlatformDisplay(from: platform)
            ...
            items.append(
                ContainerListItem(
                    ...
                    status: valueString(status, keys: ["status", "summary"])
                        ?? valueString(lookup, keys: ["status", "summary"]),
                    created: valueString(configuration, keys: ["creationdate", "created"]),
                    platform: platformDisplay,
                    hostname: valueString(networkOptions, keys: ["hostname"]),
                    cpuCount: valueInt(resources, keys: ["cpus"]),
                    memoryInBytes: valueInt64(resources, keys: ["memoryinbytes", "memory"]),
                    publishedPorts: publishedPortDisplays(from: valueDictionaryArray(configuration, key: "publishedports")),
                    command: commandText(from: initProcess),
                    rosetta: valueBool(configuration, keys: ["rosetta"]),
                    readOnly: valueBool(configuration, keys: ["readonly"]),
                    role: labels["com.apple.container.resource.role"]
                )
            )
        }
    ...
    private func decodeImageList(from output: String) -> NonCriticalDecodeResult<[ImageListItem]> {
        if let typedResult = decodeTypedImageList(from: output) {
            return typedResult
        }

        return decodeLooseImageList(from: output)
    }

    private func decodeTypedImageList(from output: String) -> NonCriticalDecodeResult<[ImageListItem]>? {
        guard let data = output.data(using: .utf8),
              let rows = try? JSONDecoder().decode([ImageListRow].self, from: data)
        else {
            return nil
        }

        let items = rows.compactMap { row -> ImageListItem? in
            guard let id = row.id ?? row.configuration?.descriptor.digest,
                  let reference = row.configuration?.name
            else {
                return nil
            }
            ...
            return ImageListItem(
                id: id,
                reference: reference,
                size: row.configuration?.descriptor.size.map(String.init) ?? row.variants.first?.size.map(String.init),
                created: row.configuration?.creationDate ?? row.variants.first?.config?.created,
                mediaType: row.configuration?.descriptor.mediaType,
                variantCount: row.variants.count,
                platforms: row.platformDisplays,
                totalVariantSizeBytes: row.totalVariantSizeBytes
            )
        }

        var diagnostics = DecodeDiagnostics.clean
        diagnostics.droppedRecords = rows.count - items.count

        if rows.isEmpty {
            return .parsed(value: [], diagnostics: diagnostics)
        }
    ...
    private func decodeLooseImageList(from output: String) -> NonCriticalDecodeResult<[ImageListItem]> {
        guard let rows = parseJSONArrayRows(from: output) else {
            return .raw(
                output: output,
                diagnostics: DecodeDiagnostics(
        ...
        for row in rows {
            ...
            let configurationDescriptor = nestedDictionary(configuration, key: "descriptor")
            let variants = valueDictionaryArray(lookup, key: "variants")
            let firstVariant = variants.first.map(normalizeKeys)
            let firstVariantConfig = nestedDictionary(firstVariant, key: "config")

            guard
                let id = valueString(lookup, keys: ["id", "digest", "imageid"])
                    ?? valueString(descriptor, keys: ["digest"])
                    ?? valueString(configurationDescriptor, keys: ["digest"]),
                let reference = valueString(lookup, keys: ["name", "reference", "repository", "tag"])
                    ?? valueString(configuration, keys: ["name", "reference", "repository", "tag"])
            else {
                diagnostics.droppedRecords += 1
                continue
            }
            ...
            items.append(
                ImageListItem(
                    ...
                    size: valueString(lookup, keys: ["fullsize", "size", "virtualsize"])
                        ...
                        ?? valueInt64(configurationDescriptor, keys: ["size"]).map(String.init),
                    created: valueString(lookup, keys: ["created", "createdat"])
                        ?? valueString(configuration, keys: ["creationdate", "created", "createdat"])
                        ?? valueString(firstVariantConfig, keys: ["created"]),
                    mediaType: valueString(descriptor, keys: ["mediatype"])
                        ?? valueString(configurationDescriptor, keys: ["mediatype"]),
                    variantCount: variants.count,
                    platforms: variants.compactMap { platformDisplay(from: normalizeKeys($0)) },
                    totalVariantSizeBytes: totalVariantSize(from: variants)
                )
            )
        }
    ...
    func decodeImageListForTesting(_ output: String) -> NonCriticalDecodeResult<[ImageListItem]> {
        decodeImageList(from: output)
    }

    func decodeContainerListForTesting(_ output: String) -> NonCriticalDecodeResult<[ContainerListItem]> {
        decodeContainerList(from: output)
    }

    func decodeContainerInspectForTesting(id: String, output: String) -> ContainerInspectSnapshot {
        decodeContainerInspect(id: id, output: output)
    }

    func decodeContainerStatsForTesting(
        _ output: String,
        capturedAt: Date = .now
    ) -> [ContainerStatsSample] {
        decodeContainerStats(from: output, capturedAt: capturedAt)
    }
    ...
    private func decodeContainerStats(
        from output: String,
        fallbackID: String? = nil,
        capturedAt: Date = .now
    ) -> [ContainerStatsSample] {
        guard let rows = parseJSONArrayRows(from: output) else { return [] }

        return rows.compactMap { row in
            let lookup = normalizeKeys(row)
            guard let containerID = valueString(lookup, keys: ["id"]) ?? fallbackID else {
                return nil
            }
    ...
    private func valueBool(_ dictionary: [String: Any], keys: [String]) -> Bool? {
        for key in keys {
            if let value = dictionary[key] {
    ...
    private func platformDisplay(from variant: [String: Any]) -> String? {
        let platform = nestedDictionary(variant, key: "platform")
        let config = nestedDictionary(variant, key: "config")

        guard let os = valueString(platform, keys: ["os"]) ?? valueString(config, keys: ["os"]),
              let architecture = valueString(platform, keys: ["architecture"]) ?? valueString(config, keys: ["architecture"])
        else {
            return nil
        }

        var parts = [os, architecture]
        if let variant = valueString(platform, keys: ["variant"]) ?? valueString(config, keys: ["variant"]) {
            parts.append(variant)
        }
        return parts.joined(separator: "/")
    }

    private func containerPlatformDisplay(from platform: [String: Any]) -> String? {
        guard
            let os = valueString(platform, keys: ["os"]),
            let architecture = valueString(platform, keys: ["architecture"])
        else {
            return nil
        }

        var parts = [os, architecture]
        if let variant = valueString(platform, keys: ["variant"]) {
            parts.append(variant)
        }
        return parts.joined(separator: "/")
    }

    private func publishedPortDisplays(from ports: [[String: Any]]) -> [String] {
        ports.compactMap { rawPort in
            let port = normalizeKeys(rawPort)
            guard let containerPort = valueInt(port, keys: ["containerport"]) else { return nil }
            let hostAddress = valueString(port, keys: ["hostaddress"]) ?? "0.0.0.0"
            let hostPort = valueInt(port, keys: ["hostport"]).map(String.init) ?? String(containerPort)
            let proto = valueString(port, keys: ["proto", "protocol"]) ?? "tcp"
            return "\(hostAddress):\(hostPort)->\(containerPort)/\(proto)"
        }
    }

    private func totalVariantSize(from variants: [[String: Any]]) -> Int64? {
        let total = variants.compactMap { valueInt64(normalizeKeys($0), keys: ["size"]) }.reduce(Int64(0), +)
        return total > 0 ? total : nil
    }
...
extension ContainerListItem {
    ...
    var resourceSummary: String {
        let cpuText = cpuCount.map { "\($0) CPU" } ?? "Default CPU"
        let memoryText = memoryInBytes.map { ByteCountFormatter.string(fromByteCount: $0, countStyle: .memory) } ?? "Default memory"
    ...
    var portsDisplay: String {
        publishedPorts.isEmpty ? "No published ports" : publishedPorts.joined(separator: ", ")
    }

    var runtimeBadges: [String] {
        var values: [String] = []
        if rosetta == true { values.append("Rosetta") }
        if readOnly == true { values.append("Read-only") }
        if let role, !role.isEmpty { values.append(role.capitalized) }
        return values
    }

    var matchesSearchText: String {
        [id, name, state, status ?? "", image ?? "", platform ?? "", hostname ?? "", role ?? "", portsDisplay]
            .joined(separator: " ")
            .lowercased()
    }
}

extension ContainerInspectSnapshot {
    var resourceSummary: String {
        let cpuText = cpuCount.map { "\($0) CPU" } ?? "Default CPU"
        let memoryText = memoryBytes.map { ByteCountFormatter.string(fromByteCount: $0, countStyle: .memory) } ?? "Default memory"
        return "\(cpuText) / \(memoryText)"
    }
