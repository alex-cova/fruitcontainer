...
struct OnboardingView: View {
        ...
        VStack(alignment: .leading, spacing: 18) {
            HStack(alignment: .top, spacing: 18) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    Text("Welcome to Fruit Container")
                        .font(.system(size: 32, weight: .bold))

                    Text("Fruit Container gives you a focused dashboard for containers, images, networks, volumes, and runtime health on macOS.")
                        .font(.title3)
        ...
        VStack(alignment: .leading, spacing: 16) {
            HStack(alignment: .top, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("After setup, click the menu bar icon to reopen Fruit Container and get back to the full dashboard when you need it.")
                        .font(.callout)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                }
    ...
    private var openAtLoginDescription: String {
        if loginLaunchController.needsApproval {
            return "macOS needs your approval in System Settings before Fruit Container can launch automatically."
        }

        if openAtLogin {
            if showMenuBarExtra {
                return "Fruit Container will launch at login and stay in the menu bar."
            }

            return "Fruit Container will launch at login and open the main window because the menu bar extra is turned off."
        }

        return "Fruit Container only launches when you open it yourself."
    }
